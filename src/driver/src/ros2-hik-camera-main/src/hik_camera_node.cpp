...
public:
  /*!
   * @class HikCameraNode
   * @brief 一个用于管理Hik摄像头的ROS 2节点
   *
   * 这个类会在初始化时枚举可用的USB摄像头设备，并建立与Hik摄像头的连接。其主要功能包括：
   * 1. 打开设备并获取相机图像信息（长宽及像素格式等）。
   * 2. 创建并配置图像的转换参数，用于将相机图像转换为RGB格式。
   * 3. 发布图像话题，并将相机信息发布到指定的话题上。
   * 4. 配合CameraInfoManager加载相机的配置文件（如果有效），为外部提供相机标定等信息。
   * 5. 通过独立的捕获线程不断抓取图像，并在user要求的QoS质量配置下发布该图像流。
   * 6. 如果抓帧出现一定次数的失败，会停止并发出致命错误提示，最终关闭ROS节点。
   *
   * 参数：
   * - @param use_sensor_data_qos：是否使用sensor_data的QoS配置。
   * - @param camera_name：相机名称，用于在CameraInfoManager中进行标识。
   * - @param camera_info_url：相机info的配置文件URL，支持package://等ROS资源格式。
   */
